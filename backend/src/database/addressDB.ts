...
export const getAddressDB = async (userId: number) => {
  const getQuery = "SELECT * FROM address WHERE user_id = $1";
...
export const addAddressDB = async (
  ...
  const addQuery =
    "INSERT INTO address (address_line1, address_line2, city, state, zip_code, phone, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *";
...
export const removeAddressDB = async (id: number) => {
  const removeQuery = "DELETE FROM address WHERE id = $1";
